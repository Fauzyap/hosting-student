...
if (isset($_GET['cari'])) {
    ...
    if ($radiovalue == "ttl") {
        $query = mysqli_query($conn, "SELECT * FROM siswa WHERE ttl LIKE '%" . $cari . "%' ");
    }
    //radio ceklis = semua
    if ($radiovalue == "semua") {
        $query = mysqli_query($conn, "SELECT * FROM siswa WHERE nis LIKE '%" . $cari . "%' OR nama LIKE '%" . $cari . "%' OR ttl LIKE '%" . $cari . "%' ");
    }
